...
numChnls, int numFilts, int HtQBsWTCGEkpylRklilw, int IAlDgIFcchbwRGBSfVfA, int 
DSsxcjIrUgZCKZovyNQf, int CpMjJjtGOeWOzwxpAAQP, int FeVcBgtQmTLtmnNcJGMY, 
int FrpxvsDMwwgbpqHXWxmN, int AIXLuRgdeiqpaCehGSYD, int 
ATEikvMQPqBefhJzjzhc, int , const char* zzWugmJRYlNEuAzHMpeQ, const 
char* OVOphSOolqRQDDoKPwxy) : MWCNNLayerImpl(layer, ntwk_impl) , 
AwZQzUhuWVLGrWgLHRuM(filt_H) , AzTsxYcYjIEJsGQbeYHm(filt_W) , 
CGbFsczkgkhjcHoCKzBx(numGrps) , HtQBsWTCGEkpylRklilw(HtQBsWTCGEkpylRklilw) , 
...
numChnls * numFilts * CGbFsczkgkhjcHoCKzBx * AwZQzUhuWVLGrWgLHRuM * 
AzTsxYcYjIEJsGQbeYHm; veFyKKHbdqBIvQLYBqfF = (float*)calloc(eybNKlJCSDUvsznWynwK, 
sizeof(float)); NMMfJylfQjiIUAKhXCJb = (float*)calloc(numFilts * CGbFsczkgkhjcHoCKzBx, 
sizeof(float)); loadWeights(zzWugmJRYlNEuAzHMpeQ, eybNKlJCSDUvsznWynwK); 
loadBias(OVOphSOolqRQDDoKPwxy, numFilts * CGbFsczkgkhjcHoCKzBx); auto 
...
dnnl::memory(bias_md, *euppfEoiaoCTcVgRPVhA->eng)); conv_bias_md = 
...
{CGbFsczkgkhjcHoCKzBx, numFilts, numChnls, AwZQzUhuWVLGrWgLHRuM, 
...
void MWFusedConvReLULayerImpl::propagateSize() { clearLayerMapsAndVectors(); 
...
opTensor->getChannels(); int WIxRBCJtmETvfxpuRuus = 
convLayer->getInPlaceIndex(0); if (numInputs == 2 && WIxRBCJtmETvfxpuRuus != 
-1) { assert(WIxRBCJtmETvfxpuRuus == 1); } dnnl::primitive_attr attr; 
dnnl::post_ops ops; float scale = 1.0f; float alpha = 0.0f; float beta = 1.0f;  
if (numInputs == 2) { ops.append_sum(1.0f); } ops.append_eltwise(scale, 
dnnl::algorithm::eltwise_relu, alpha, beta); attr.set_post_ops(ops); 
conv_src_md = std::unique_ptr<dnnl::memory::desc>(new dnnl::memory::desc( {n, 
c, h, w}, dnnl::memory::data_type::f32, dnnl::memory::format_tag::any)); 
conv_dst_md = std::unique_ptr<dnnl::memory::desc>( new dnnl::memory::desc({n, 
num_filts, opTensor->getHeight(), opTensor->getWidth()}, 
dnnl::memory::data_type::f32, dnnl::memory::format_tag::any)); conv_d = 
with_bias ? std::unique_ptr<dnnl::convolution_forward::desc>(new 
...
*conv_src_memory}}); pipeline.push_back(*conv_reorder_src); } if 
...
MWMkldnnUtils::getLayerMemoryPrimitive(additionIpTensor); if 
(MWMkldnnUtils::getLayerMemoryPrimitive(additionIpTensor)->get_desc() != 
conv_pd->dst_desc()) { additionMemPrimitive = 
std::make_shared<dnnl::memory>(conv_pd->dst_desc(), *euppfEoiaoCTcVgRPVhA->eng); 
...
*MWMkldnnUtils::getLayerMemoryPrimitive(additionIpTensor)}, {DNNL_ARG_TO, 
...
} pipeline.push_back(*conv); return; } void MWFusedConvReLULayerImpl::predict() 
{ MWCNNLayer* convLayer = getLayer(); int numInputs = 
static_cast<int>(convLayer->getNumInputs()); int WIxRBCJtmETvfxpuRuus = 
...
WIxRBCJtmETvfxpuRuus == -1) { MWTensorBase* ipTensor2Base = 
convLayer->getInputTensor(WIxRBCJtmETvfxpuRuus);  MWTensor<float>* ipTensor2 = 
...
MWMkldnnUtils::getLayerMemoryPrimitive(ipTensor2)->get_desc().get_size()); 
...
MWFusedConvReLULayerImpl::cleanup() { if (veFyKKHbdqBIvQLYBqfF) { free(veFyKKHbdqBIvQLYBqfF); 
...
MWFusedConvReLULayerImpl::loadBias(const char* SGsAudmgjmvcUXzzrUtf, int 
eybNKlJCSDUvsznWynwK) { float* NZjOkZPwLzQsdEVkwMcX = NMMfJylfQjiIUAKhXCJb; FILE* SUleyRyvAggTFnSdxLru = 
MWCNNLayer::openBinaryFile(SGsAudmgjmvcUXzzrUtf); 
