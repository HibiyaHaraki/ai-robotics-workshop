...
// Generated 08-Apr-2021 16:21:22
  ...
  namespace matlab
  {
    ...
    void NodeInterface::initialize(int argc, char ** argv)
    {
      try {
        ...
        ROS_INFO("** Starting the model \"ros_line_follower\" **\n");

        // initialize the model which will initialize the publishers and subscribers
        rtmSetErrorStatus(ros_line_follower_M, (NULL));
        ros_line_follower_initialize();
    ...
    boolean_T NodeInterface::getStopRequestedFlag(void)
    {
      ...
      return (!(rtmGetErrorStatus(ros_line_follower_M)
                == (NULL)));

#else

      return (!(rtmGetErrorStatus(ros_line_follower_M)
                == (NULL)) || rtmGetStopRequested(ros_line_follower_M));
    ...
    {
      mRunModel = (rtmGetErrorStatus(ros_line_follower_M) ==
                   (NULL));
      while (mRunModel) {
        mBaseRateSem.wait();
